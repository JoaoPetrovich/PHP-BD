-- CREATE TRIGGER nomeDoTrigger Momento ComandoDML ON tabela FOR EACH ROW
-- Momentos: BEFORE (antes) ou AFTER (depois)
-- Comandos DML: INSERT, UPDATE ou DELETE
-- Sugestão para nome do Trigger: momentoComandoTabela
...
DELIMITER $$
CREATE TRIGGER antesAlterarCidade 
...
DELIMITER $$
CREATE TRIGGER depoisInserirCidade 
...
BEGIN
    ...
    VALUES ('INSERT', 'cidade', NEW.IDCIDADE);
...
DELIMITER $$
CREATE TRIGGER depoisAlterarCidade 
...
DELIMITER $$
CREATE TRIGGER depoisExcluirCidade 
...
DELIMITER $$
CREATE TRIGGER antesInserirBkpcidade 
BEFORE INSERT ON bkpcidade 
FOR EACH ROW
BEGIN
    SET NEW.DTBKP = CURRENT_DATE;
    SET NEW.HRBKP = CURRENT_TIME;
...
DELIMITER $$
CREATE TRIGGER antesInserirMarca
BEFORE INSERT ON marca FOR EACH ROW
BEGIN
    SET NEW.NOME = UPPER(NEW.NOME);
    SET NEW.PRODUTOS =UPPER(NEW.PRODUTOS);
	SET NEW.DTINSERT = CURRENT_DATE;
    SET NEW.HRINSERT = CURRENT_TIME;
END $$
DELIMITER ;
...
DELIMITER $$
CREATE TRIGGER antesAlterarMarca
...
INSERT INTO histórico (ACAO,TABELA,ID) VALUES ("INSERT","marca",NEW.IDMARCA);
END $$
DELIMITER;
...
INSERT INTO bkpMarca (IDMARCA,NOME,PRODUTOS,DTINSERT,HRINSERT,DTUPDATE,HRUPDATE)
VALUES (NEW.IDMARCA,NEW.NOME,NEW.PRODUTOS,NEW.DTINSERT,NEW.HRINSERT,NEW.DTUPDATE,NEW.HRUPDATE);
INSERT INTO histórico (ACAO,TABELA,ID) VALUES ("UPDATE","marca",NEW.IDMARCA);
END $$
DELIMITER;
...
BEGIN
INSERT INTO bkpMarca (IDMARCA,NOME,PRODUTOS,DTINSERT,HRINSERT,DTUPDATE,HRUPDATE)
VALUES (OLD.IDMARCA,OLD.NOME,NEW.PRODUTOS,OLD.DTINSERT,OLD.HRINSERT,OLD.DTUPDATE,OLD.HRUPDATE);
INSERT INTO histórico (ACAO,TABELA,ID) VALUES ("DELETE","marca",OLD.IDMARCA);
END $$
DELIMITER;
...
BEGIN
	...
	-- Calcular e atualizar o número de PRODUTOS da unidade
	IF(NEW.IDUNIDADE<>OLD.IDUNIDADE) THEN
		CALL calcularProdutosUnidade(NEW.IDUNIDADE);
